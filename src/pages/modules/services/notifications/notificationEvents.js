...
export async function notifyCreditCreated({
  ...
  await createNotification({
    ...
    message:
      `Se creó un crédito para ${client} por $${Number(amount).toLocaleString()}`
...
export async function notifyPaymentRegistered({
  ...
  await createNotification({
    ...
    message:
      `${client} realizó un pago de $${Number(amount).toLocaleString()}`
...
export async function notifyClientOverdue({
  ...
  await createNotification({
    ...
    message:
      `${client} presenta cuotas vencidas.`
...
export async function notifyTodayCollection({
  ...
  await createNotification({
    ...
    message:
      `Hoy corresponde cobrar a ${client}.`
...
export async function notifyFollowUp({
  ...
  await createNotification({
    ...
    message:
      `${client} tiene seguimiento programado para ${date}.`
